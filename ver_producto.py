...
# --- NUEVA FUNCIÓN: Solo imágenes en el ZIP ---
def exportar_imagenes(producto):
    urls = []
    ...
    return zip_buffer.getvalue(), producto.get("nombre_producto", "imagenes_producto"), errores

with colbtn3:
    zip_data, nombre_zip, errores = exportar_imagenes(producto)
    if errores:
        st.warning("Algunas imágenes no se pudieron descargar:\n\n" + "\n".join(errores))
    nombre_seguro = re.sub(r'[^\w\-_.]', '_', nombre_zip)
    zip_buffer = BytesIO(zip_data)
    st.download_button(
        label="⬇️ Exportar ZIP de imágenes",
